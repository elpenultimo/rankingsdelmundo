...
import { getComparableMetricsForCity, type Metric } from "../../../../lib/compare";
import {
  CompareMetricRow,
  ogCategoryChips,
  ogTheme,
...
export default function OpenGraphImage({ params }: { params: { slug: string } }) {
  ...
  const entityA = parsed ? getEntityBySlug("ciudad", parsed.aSlug) : undefined;
  const entityB = parsed ? getEntityBySlug("ciudad", parsed.bSlug) : undefined;

  const metricsA = entityA ? getComparableMetricsForCity(entityA.name) : [];
  const metricsB = entityB ? getComparableMetricsForCity(entityB.name) : [];
