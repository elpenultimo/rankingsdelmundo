...
import { isRankingYear } from "../../../../../data/taxonomy";
...
export default function OpenGraphImage({ params }: { params: { slug: string; year: string } }) {
  const ranking = getRankingBySlug(params.slug);
  const isValidYear = isRankingYear(params.year);
  const title = ranking && isValidYear ? `${ranking.title} — ${params.year}` : "Vista no disponible";
  const subtitle = ranking && isValidYear
    ? `Top 20 • Actualizado ${params.year} • Índices referenciales`
